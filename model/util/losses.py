...
def mse_loss(image, reference):
    with tf.variable_scope('l2_loss'):
        reduce_axes = list(range(1, len(image.shape)))  # auto infer axes
        loss = tf.reduce_mean(tf.squared_difference(image, reference), axis=reduce_axes)
        return tf.reduce_mean(loss)

def mae_loss(image, reference):
    with tf.variable_scope('l1_loss'):
        reduce_axes = list(range(1, len(image.shape)))
        loss = tf.reduce_mean(tf.abs(image - reference), axis=reduce_axes)
        return tf.reduce_mean(loss)
...
def edge_loss(image, reference):
    '''
    image, reference: tensors of shape [B, D, H, W, C] or [B, H, W, C]
    Computes edge-wise MSE loss using Sobel gradients
    '''
